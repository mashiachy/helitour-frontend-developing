...
var DatePicker = unwrapExports(vue2Datepicker);
...
const app = new Vue({
  ...
  watch: {
    ...
    city (v) {
      ...
      if (v) {
        ...
        this.warehouses = [];
        axios$1.post('/api/delivery/warehouse.json', { id: this.city }).then( ({ data: warehouses }) => {
        // axios.get('/warehouse.json').then( ({ data: warehouses }) => {
          // this.warehouses = warehouses.map(({ id: strId, text: name }, i) => ({ strId, name, id: i+1 }));
          this.warehouses = warehouses;
          this.warehouseFuse = new fuse_common(this.warehouses, { 
            keys: ['text'],
            threshold: 0.7,
          });
  ...
  async created () {
    const { data } = await axios$1.get('/helitours_info.json?lang='+document.querySelector('html').getAttribute('lang'));
